...
__decorate([
    (0, node_stage_1.Option)({ describe: "Remove all undefined files from S3" }),
    __metadata("design:type", Boolean)
], SpaBuildOptions.prototype, "purge", void 0);
__decorate([
    (0, node_stage_1.Option)({ describe: "Replace all files even if not changed" }),
    __metadata("design:type", Boolean)
...
exports.command = {
    ...
    builder: async (y) => {
        return y
            .options((0, node_stage_1.getYargsOptions)(SpaBuildOptions))
            .middleware(async (_argv) => {
            return (await (0, node_stage_1.loadYargsConfig)(SpaBuildOptions, _argv, "spaDeploy"));
        }, true);
    },
    handler: async (_argv) => {
        const argv = (await _argv);
        (0, node_stage_1.logBanner)(`SPA Build ${(0, version_helper_1.getVersion)()}`);
        const pwd = argv.pwd;
        ...
        if (stage) {
            ...
            (0, node_stage_1.logVariable)("STAGE", argv.stage);
        }
        if (!argv.ci) {
            (0, node_stage_1.logInfo)("Running Interactively");
        }
        const verbose = !!argv.verbose;
        const gitChanges = await (0, node_stage_1.getGitChanges)(pwd);
        if (gitChanges !== "") {
        ...
        (0, node_stage_1.logVariable)("app__buildPath", buildPath);
        if (!fs_1.default.lstatSync(buildPath).isDirectory()) {
            (0, node_stage_1.logError)(`Build path ${buildPath} is not a directory.`);
            return process_1.default.exit(1);
        }
        const release = argv.release;
        (0, node_stage_1.logBanner)("App Environment");
        ...
        if (!awsRegion) {
            (0, node_stage_1.logError)(`AWS Region is not set`);
            return process_1.default.exit(1);
        }
        (0, node_stage_1.logVariable)("app__aws__region", awsRegion);
        if (spaDeploy?.aws?.endpoint) {
            (0, node_stage_1.logVariable)("app__aws__endpoint", spaDeploy?.aws?.endpoint);
        }
        const deployBucket = spaDeploy?.s3?.bucket;
        if (!deployBucket) {
            (0, node_stage_1.logError)(`S3 Deploy Bucket is not set`);
            return process_1.default.exit(1);
        }
        else {
            (0, node_stage_1.logVariable)("app__s3__bucket", spaDeploy?.s3?.bucket);
        }
        if (spaDeploy?.s3?.prefix) {
            (0, node_stage_1.logVariable)("app__s3__prefix", spaDeploy?.s3?.prefix);
        }
        const s3Options = {
            region: awsRegion,
            bucket: deployBucket,
        ...
        if (indexFiles.length > 0) {
            ...
            for (const item of indexFiles) {
                ...
                if (data.match('<script id="env-data">')) {
                    ...
                    (0, node_stage_1.logWarning)(`Could not find <script id="env-data"> in ${item.key}. Injecting at end of HEAD.`);
                    item.data = data.replace(/<\/head>/, injectedData + `</head>`);
                }
                else {
                    (0, node_stage_1.logWarning)(`Could not find injection point in ${item.key}`);
                    continue;
                }
        ...
        // s3 sync plan
        (0, node_stage_1.logBanner)(`S3 Sync Plan`);
        ...
        if (cloudfrontInvalidations.length > 0) {
            (0, node_stage_1.logBanner)(`Cloudfront invalidations`);
            for (const i of cloudfrontInvalidations) {
                console.log(i);
            }
        }
        // deploy
        (0, node_stage_1.logBanner)(`Deploy`);
        if (cloudfrontId.length < 1 && cloudfrontInvalidations.length > 0) {
            (0, node_stage_1.logWarning)("No cloudfront set - will not invalidate cache!");
        }
        if (!plan.items.some((x) => [sync_helper_1.SyncAction.create, sync_helper_1.SyncAction.update, sync_helper_1.SyncAction.delete].includes(x.action))) {
            (0, node_stage_1.logInfo)("Nothing to do!");
            return;
        }
        if (!argv.ci) {
            if (!(await (0, node_stage_1.confirm)("Press enter to deploy..."))) {
